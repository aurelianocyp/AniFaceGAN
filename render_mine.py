...
def staged_forward(z_id, z_exp, noise, generator_ddp, deform_ddp, neutral_face_flag, stage, alpha, metadata, opt):
    ...
    with torch.no_grad():
        ...
        for split in range(metadata['batch_split']):
            ...
            if not psi == 1:
                truncated_frequencies = generator_ddp.avg_frequencies + psi * (
                        raw_frequencies - generator_ddp.avg_frequencies)
                truncated_phase_shifts = generator_ddp.avg_phase_shifts + psi * (
                        raw_phase_shifts - generator_ddp.avg_phase_shifts)
            else:
            ...
            # ------------------------------------------- Network Forward ----------------------------------------------------------
            with torch.no_grad():
                wp_sample_deform, wp_inter_back_deform, levels, w_ray_origins, w_ray_directions, pitch, yaw, cam2world = generator_ddp.generate_points(
...
def generate_img(generator, deform_net, z_id, z_exp, z_noise, flag, kwargs):
    with torch.no_grad():
        img, depth_map, intersections_deform, intersections_canonic, _, weight, weighted_points = staged_forward(z_id,
                                                                                                                 z_exp,
                                                                                                                 z_noise,
                                                                                                                 generator,
                                                                                                                 deform_net,
...
def read_latents(name):
    ...
    latent_pose = torch.from_numpy(latents['angle']).float()[0, ...]

    return latent_id, latent_exp, latent_pose
...
if __name__ == '__main__':
    ...
    parser.add_argument('--generator_file', type=str,
                        default='results/20220301-064028_warm_up_deform_0_switch_interval_2_DIF_lambda_4000_ths_0.000100_gen_gt/generator.pth')
    parser.add_argument('--deform_file', type=str,
                        default='results/20220301-064028_warm_up_deform_0_switch_interval_2_DIF_lambda_4000_ths_0.000100_gen_gt/dif.pth')
    ...
    # facerecon
    parser.add_argument('--checkpoints_dir', type=str, default='./FaceRecon_Pytorch/checkpoints',
    ...
    parser.add_argument('--init_path', type=str,
                        default='./FaceRecon_Pytorch/checkpoints/init_model/resnet50-0676ba61.pth')
    ...
    generator = getattr(generators, metadata['generator'])(generator_core, metadata['latent_dim'], **metadata).to(
        device)
    ...
    flag=0
    for mat_path in tqdm(annotations):
        #if flag>100:
         #   break
        flag=flag+1
        exp_files = []
        trajectory = []  # If you want to change the rendering camera pose, modify here

        latent_id, latent_exp, latent_pose = read_latents(mat_path)
        # print("latent pose", latent_pose.shape)  # latent pose torch.Size([3])
        for t in range(num_steps):
            pitch = latent_pose[1]+math.pi / 2
            yaw = latent_pose[0] + math.pi / 2
            fov = 12
            trajectory.append((pitch, yaw, fov))
        ratio = np.linspace(0, 1.0, num_steps)
        for pp in range(num_steps):
            exp_files.append(latent_exp.unsqueeze(0))

        num_expression = len(exp_files)
        latent_exp = latent_exp.unsqueeze(0).to(device)
        import skvideo

        skvideo.setFFmpegPath("/root/miniconda3/bin")
        import skvideo.io
        ...
        with torch.no_grad():
            ...
            for seed in seeds:
                ...
                writer = FFmpegWriter(os.path.join(opt.output_dir, output_name),
                                      outputdict={'-pix_fmt': 'yuv420p', '-crf': '21'}, verbosity=10)
                # fourcc = cv2.VideoWriter_fourcc(*'mp4v')
                # out = cv2.VideoWriter(os.path.join(opt.output_dir, output_name), fourcc, 25, (800, 800))
                ...
                for exp_id in range(num_steps):
                    ...
                    save_image(tensor_img, os.path.join(cnt_output_dir, f'pred_img_{flag}.png' ),
